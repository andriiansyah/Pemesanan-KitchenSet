<?php 
    ...
    $transfer_atas_nama = $_POST['transfer_atas_nama'];
    $bukti_tranfer = $_POST['bukti_tranfer'];
    $tgl_pembayaran = $_POST['tgl_pembayaran'];
    $foto_nama = $_FILES['bukti_tranfer']['name'];
    $foto_size = $_FILES['bukti_tranfer']['size'];
    ...
    $file_tmp = $_FILES['bukti_tranfer']['tmp_name'];   
    ...
    if(in_array($ekstensi, $ekstensi_izin) === true)  {
      ...
      $get_foto = "SELECT bukti_tranfer FROM pembayaran WHERE id_pembayaran = '$id_pembayaran'";
      $data_foto = mysqli_query($conn, $get_foto); 
      // Mengubah data yang diambil menjadi Array
      $foto_lama = mysqli_fetch_array($data_foto);

      // Menghapus Foto lama didalam folder FOTO
      unlink("dashboard/images/bukti/".$foto_lama['bukti_tranfer']);    

      // Memindahkan File kedalam Folder "FOTO"
      move_uploaded_file($file_tmp, 'dashboard/images/bukti/'.$foto_nama_new);

      // Query untuk memasukan data kedalam table SISWA
      $query = mysqli_query($conn, "UPDATE pembayaran SET transfer_atas_nama='$transfer_atas_nama', bukti_tranfer='$foto_nama_new', status='Konfirmasi Pembayaran', tgl_pembayaran='$tgl_pembayaran' WHERE id_pembayaran = '$id_pembayaran'");
    ...
    // $edit = mysqli_query($conn, "UPDATE pembayaran SET transfer_atas_nama='$transfer_atas_nama', nama_admin='$nama_admin', no_telp_admin='$no_telp_admin', alamat_admin = '$alamat_admin' WHERE id_admin = $id_admin");
    // if($edit) {
